...
@inject
def organizations_search(organization_service: OrganizationService = Provide[Container.organization_service]) -> Response:
...
@inject
def get_organization(organization_id: str, organization_service: DatasetService = Provide[Container.organization_service]) -> Response:
...
@inject
def reuses_search(reuse_service: ReuseService = Provide[Container.reuse_service]) -> Response:
    page = request.args.get('page', 1, type=int)
    page_size = request.args.get('page_size', 20, type=int)
    query_text = request.args.get('q', None)
...
@inject
def get_reuse(reuse_id: str, reuse_service: ReuseService = Provide[Container.reuse_service]) -> Response:
